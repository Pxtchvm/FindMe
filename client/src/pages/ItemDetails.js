...
const ItemDetails = () => {
    ...
    const { user } = useAuth();
    const { currentItem, loading, getItemById, deleteItem, claimItem, processClaim } = useItems();
    ...
    const isOwner = currentItem && user && currentItem.reportedBy._id === user.id;

    // Check if user can claim (not owner, item available, and not already claimed)
    const canClaim = currentItem && user &&
        currentItem.reportedBy._id !== user.id &&
        currentItem.status === 'available';

    // Check if user can process a claim
    const canProcessClaim = currentItem && isOwner && currentItem.status === 'pending';
    ...
    const handleProcessClaim = async (approve) => {
        try {
            ...
            setSuccess(approve ? 'Claim approved successfully' : 'Claim rejected successfully');
    ...
    return (
        <Container maxWidth="md">
                    ...
                    <Grid item xs={12} md={6}>
                        <Card>
                            <CardMedia
                                component="img"
                                height="300"
                                image={currentItem.photoUrl || DEFAULT_IMAGE}
                    ...
                    <Grid item xs={12} md={6}>
                        <Box sx={{ mb: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                                <Chip
                                    label={currentItem.type === 'lost' ? 'Lost Item' : 'Found Item'}
                                    color={currentItem.type === 'lost' ? 'error' : 'info'}
                                    size="small"
                                />
                        ...
                        <Box sx={{ mb: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                ...
                                <Typography variant="body2">
                                    <strong>Date:</strong> {formatDate(currentItem.date)}
                                </Typography>
                            </Box>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                ...
                                <Typography variant="body2">
                                    <strong>Reported by:</strong> {currentItem.reportedBy.firstName} {currentItem.reportedBy.lastName}
                                </Typography>
                            </Box>
                            ...
                            {currentItem.contactInfo && (
                                <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                    ...
                                    <Typography variant="body2">
                                        <strong>Contact:</strong> {currentItem.contactInfo}
                                    </Typography>
                                </Box>
                            )}
                        </Box>
            ...
            <Dialog open={claimDialogOpen} onClose={() => setClaimDialogOpen(false)}>
                ...
                <DialogContent>
                    <DialogContentText>
                        By submitting a claim, you are stating that this item belongs to you. Your claim will be reviewed by the person who reported the item.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                ...
                <DialogContent>
                    <DialogContentText>
                        This action cannot be undone. Are you sure you want to delete this item report?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
